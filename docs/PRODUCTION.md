...
Railway is a common deploy target for portfolio client projects. See **[docs/RAILWAY.md](RAILWAY.md)** for the full guide.

### Service setup

1. Create a **NestJS service** from this repo (Dockerfile + [`railway.toml`](../railway.toml))
2. Add **MySQL** and **Redis** plugins
3. Copy variables from [`.env.railway.example`](../.env.railway.example)

Railway injects `PORT` automatically — do **not** set `APP_PORT` in production.

**Startup:** [`scripts/railway-start.sh`](../scripts/railway-start.sh) runs migrations, optional seed (`RUN_SEED=true`), then starts the app.

### First deploy

Set `RUN_SEED=true` once, then `false` after admin login works.

```bash
# Optional manual seed/migrate via Railway shell
